...
server <- function(input, output, session) {
    ...
    rctInput <- eventReactive(input$go, {
        ...
        rcts_df <- as.data.frame(data$StudyFieldsResponse$StudyFields) %>%
            ...
            mutate(
                Class = ifelse(str_detect(LeadSponsorName,"Hospit"), "Hospital",
                               ifelse(LeadSponsorClass == "NIH", "NIH",
                                      ifelse(str_detect(LeadSponsorName,"Universit|Universid|School|Institut|Colleg"), "University",
                                             str_to_sentence(LeadSponsorClass)))),
                Class = ifelse(Class == "Network" | Class == "Other_gov","Other",
                               ifelse(Class == "Fed", "US Gov", Class)),
                StartYear = as.numeric(str_extract(StartDate,"\\d{4}")),
                EndYear = as.numeric(str_extract(CompletionDate,"\\d{4}"))
            )
    })
    ...
    # Top Sponsors 
    topsponInput <- eventReactive(input$go, {
        rcts_df <- rctInput()
        collabs <- collabInput()
        top_spon <- rcts_df %>% 
            ...
            rbind(collabs) %>% 
            count(label,group) %>% 
            rename(Name = label, Class = group, Sponsorships = n) %>% 
    ...
    output$class_donuts <- renderPlot({
        rcts_df <- rctInput()
        # With CTG Canonical Sponsor Classes
        ctg_sponsors <- rcts_df %>% 
            count(LeadSponsorClass) %>% 
            mutate(percent = n/nrow(rcts_df),
                   ymax = cumsum(percent),
        ...
        #Make Chart 
        ctgs_plot <- ggplot(ctg_sponsors, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=LeadSponsorClass)) +
            geom_rect() +
            geom_text( x=2, aes(y=labelPosition, label=label, color=LeadSponsorClass), size=6) + # x here controls label position (inner / outer)
            scale_fill_uchicago()+
    ...
    output$timeline <- renderStreamgraph({
        ...
        rctInput() %>% 
            ...
            streamgraph("LeadSponsorName", "n", "Year",  interpolate="linear") %>% 
    ...
    output$rcts <- DT::renderDataTable({ 
        rcts_df <- rctInput() %>% 
            select(NCTId,BriefTitle,LeadSponsorName,LeadSponsorClass,InterventionName,Condition,StartDate,CompletionDate) %>% 
            mutate_all(~replace(., . == "character(0)", NA)) 
        
    })
    
    # Tab 1: Network Diagram {kept at end becaluse long & Ugly}--------------------------------------------------
    ...
    output$network_proxy <- renderVisNetwork({
        ...
        #validate
        shiny::validate(
            need(nrow(rcts_df) != 0, "Data not currently available. Try another search.")
